	...
	<div class="p-4 md:p-8">
		...
		>
			...
			{#if catalogo.restrito}
				<div
					class="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden opacity-[0.05] select-none"
				>
					<div
						class="-rotate-45 font-mono text-6xl font-black tracking-widest text-red-900 uppercase"
						style="color: #7f1d1d;"
					>
			...
			<header class="border-b-2 border-slate-900 pb-5" style="border-color: #0f172a;">
				<div class="flex items-start justify-between gap-4">
					<div class="flex items-center gap-4">
						<div
							class="flex h-14 w-14 items-center justify-center border-2 border-slate-900 bg-slate-900 font-mono text-2xl font-black text-white"
						...
						<div>
							...
							<h1
								class="mt-0.5 font-mono text-sm font-bold text-blue-900 uppercase"
								style="color: #1e3a8a;"
							>
					...
					<div class="flex flex-col items-end gap-1 text-right font-mono text-xs">
						<div
							class="border border-slate-900 bg-slate-900 px-3 py-1 font-bold text-white"
							style="background-color: #0f172a; border-color: #0f172a; color: #ffffff;"
						>
							ID: <span class="text-xs">{relatorio.id}</span>
						</div>
						<div
							class="mt-1 text-[10px] font-bold text-slate-500 uppercase"
							style="color: #64748b;"
						>
			...
			<section
				class="mt-6 border border-slate-200 bg-slate-50 p-4"
				style="border-color: #e2e8f0; background-color: #f8fafc;"
			>
				<div
					class="mb-3 flex items-center justify-between border-b border-slate-200 pb-2 font-mono text-xs font-bold text-slate-800 uppercase"
				...
				<div class="grid grid-cols-12 gap-4 font-mono text-xs">
					<div class="col-span-12 md:col-span-4">
						<span
							...
							style="color: #64748b;">Período Selecionado</span
						>
					...
					<div class="col-span-12 md:col-span-4">
						<span
							class="block text-[10px] font-bold text-slate-500 uppercase"
							style="color: #64748b;">Servidor Emissor</span
						>
						<span class="text-sm font-bold text-slate-900" style="color: #0f172a;">{operador}</span>
					</div>
					<div class="col-span-12 md:col-span-4">
						<span
							class="block text-[10px] font-bold text-slate-500 uppercase"
							style="color: #64748b;">Data/Hora de Geração</span
						>
						<span class="text-sm font-bold text-slate-900" style="color: #0f172a;"
							>{dataHoraDisplay(relatorio.geradoEm)}</span
						>
					</div>
				</div>
			</section>
			...
			<section class="mt-6 border border-slate-200 p-4" style="border-color: #e2e8f0;">
				<div
					class="mb-3 border-b border-slate-200 pb-2 font-mono text-xs font-bold tracking-wider text-slate-800 uppercase"
					style="border-color: #e2e8f0; color: #1e293b;"
				>
					1. Resumo Executivo e Status do Registro
				</div>
				<div class="grid grid-cols-2 gap-4 font-mono md:grid-cols-4">
					<div
						class="border border-slate-200 bg-slate-50 p-3.5"
						style="border-color: #e2e8f0; background-color: #f8fafc;"
					>
						<div class="text-[10px] font-bold text-slate-500 uppercase" style="color: #64748b;">
							Tamanho do Arquivo
						</div>
						<div class="mt-1 text-xl font-black text-slate-900" style="color: #0f172a;">
							{tamanhoDisplay(relatorio.tamanhoKb)}
						</div>
						<div class="mt-1 text-[10px] font-bold text-slate-600" style="color: #475569;">
							Consolidado no Servidor
						</div>
					</div>
					...
					>
						...
						<div class="mt-1 text-xl font-black text-emerald-800" style="color: #065f46;">
							{relatorio.status}
						</div>
						<div class="mt-1 text-[10px] font-bold text-emerald-700" style="color: #047857;">
					...
					>
						...
						<div class="mt-1 text-sm font-black text-slate-900" style="color: #0f172a;">
							{catalogo.restrito ? 'USO RESTRITO' : 'PÚBLICO / INTERNO'}
						</div>
						<div class="mt-1 text-[10px] font-semibold text-slate-600" style="color: #475569;">
							Conforme RBAC UniSISM
						</div>
					</div>
					...
					>
						...
						<div class="mt-1 truncate text-sm font-black text-blue-900" style="color: #1e3a8a;">
							DIGITAL · SUS
						</div>
						<div class="mt-1 text-[10px] font-semibold text-slate-500" style="color: #64748b;">
							Verificação SHA-256
						</div>
					</div>
				</div>
			...
			<section class="mt-6 border border-slate-200" style="border-color: #e2e8f0;">
				<div
					class="flex items-center justify-between bg-slate-900 px-4 py-2.5 font-mono text-xs font-bold tracking-wider text-white uppercase"
				...
				<div class="overflow-x-auto">
					<table class="w-full border-collapse text-left font-mono text-xs">
						<thead>
							<tr
								class="border-b border-slate-200 bg-slate-100 text-[10px] font-bold tracking-wider text-slate-700 uppercase"
								style="border-color: #e2e8f0; background-color: #f1f5f9; color: #334155;"
							>
								<th class="border-r border-slate-200 p-3" style="border-color: #e2e8f0;"
									>Atributo Técnico</th
								>
								<th class="border-r border-slate-200 p-3" style="border-color: #e2e8f0;"
									>Valor Apurado</th
						...
						<tbody class="divide-y divide-slate-200">
							<tr class="hover:bg-slate-50">
								<td
									class="border-r border-slate-200 p-3 font-bold text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;">Identificador Único (ID)</td
								>
								<td
									class="border-r border-slate-200 p-3 font-bold text-blue-900"
									style="border-color: #e2e8f0; color: #1e3a8a;">{relatorio.id}</td
								>
								<td class="p-3 text-right font-bold text-slate-700" style="color: #334155;"
									>Auditável</td
								>
							</tr>
							<tr class="hover:bg-slate-50">
								<td
									class="border-r border-slate-200 p-3 font-bold text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;">Tipo de Relatório</td
								>
								<td
									class="border-r border-slate-200 p-3 text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;"
									>{relatorio.tipo} — {catalogo.titulo}</td
								>
								<td class="p-3 text-right font-bold text-slate-700" style="color: #334155;"
									>Homologado</td
								>
							...
							<tr class="hover:bg-slate-50">
								<td
									class="border-r border-slate-200 p-3 font-bold text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;">Intervalo de Apuração</td
								>
								<td
									class="border-r border-slate-200 p-3 text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;">{relatorio.periodo}</td
								>
								<td class="p-3 text-right font-bold text-slate-700" style="color: #334155;"
									>Concluído</td
								>
							...
							<tr class="hover:bg-slate-50">
								<td
									class="border-r border-slate-200 p-3 font-bold text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;">Data e Hora de Geração</td
								>
								<td
									class="border-r border-slate-200 p-3 text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;"
									>{dataHoraDisplay(relatorio.geradoEm)}</td
								>
								<td class="p-3 text-right font-bold text-emerald-800" style="color: #065f46;"
									>VÁLIDO</td
								>
							</tr>
							<tr class="hover:bg-slate-50">
								<td
									class="border-r border-slate-200 p-3 font-bold text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;">Formato e Volume</td
								>
								<td
									class="border-r border-slate-200 p-3 text-slate-900"
									style="border-color: #e2e8f0; color: #0f172a;"
									>{relatorio.formato} ({tamanhoDisplay(relatorio.tamanhoKb)})</td
								>
								<td class="p-3 text-right font-bold text-emerald-800" style="color: #065f46;"
									>CONSOLIDADO</td
								>
			...
			<section class="mt-10 border-t-2 border-slate-900 pt-6" style="border-color: #0f172a;">
				<div class="flex items-center justify-between gap-6 font-mono text-xs">
					<div>
						<div class="font-bold text-slate-900 uppercase" style="color: #0f172a;">
							Validação de Integridade do Relatório
						</div>
						<div class="mt-1 text-[10px] text-slate-500" style="color: #64748b;">
							{hashVerificacao}
						</div>
					</div>
					<div class="text-right">
						<div
							class="mb-2 ml-auto w-56 border-b border-slate-800"
