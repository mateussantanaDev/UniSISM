...
export interface IntegracaoAdminStatus {
	nome: string;
	tipo?: string | null;
...
export interface IntegracoesAdminResponse {
	status?: 'online' | 'offline' | 'degraded' | string;
	checkedAt?: string;
	integracoes?: IntegracaoAdminStatus[];
}

export interface IntegracaoAdminRequest {
	nome: string;
	url: string;
	usuario?: string | null;
	senha?: string | null;
...
export interface AdminConfiguracoes {
	...
	retencao?: Record<string, unknown>;
	[key: string]: unknown;
}
...
export class ApiClient {
	...
	constructor(baseUrl: string, tokens: TokenStorage = localStorageTokens, apiKey?: string) {
		...
		this.tokens = tokens;
		const configuredApiKey = apiKey ?? (import.meta.env ? import.meta.env.VITE_API_KEY : undefined);
		const rawApiKey =
			configuredApiKey === undefined ? 'unisism-frontend-2026-4f2b8d9e' : configuredApiKey;
		this.apiKey = rawApiKey.trim() || undefined;
		console.log(
			'[UniSISM] ApiClient initialized. baseUrl:',
			this.baseUrl,
			'apiKey length:',
			this.apiKey ? this.apiKey.length : 0,
			'key starts with:',
			this.apiKey ? this.apiKey.substring(0, 5) : 'none'
		);
		...
		if (!ct.includes('application/json')) {
			if (!res.ok) {
				...
				throw new ApiError(res.status, {
					error: { code: 'ERRO_INTERNO', message: res.statusText }
				});
			}
	...
	async getBlob(path: string): Promise<{ blob: Blob; filename: string }> {
		...
		if (!res.ok) {
			...
			try {
				...
				body = { error: { code: 'ERRO_INTERNO', message: res.statusText } };
			}
...
class EncaminhamentosApi {
	...
	async create(input: {
		...
		if (input.solicitacaoPdf) {
			fd.append(
				'solicitacao',
				input.solicitacaoPdf,
				(input.solicitacaoPdf as File).name ?? 'solicitacao.pdf'
			);
		}
	...
	aprovar(id: string, req: AprovarRequest = {}): Promise<Encaminhamento> {
		return this.api.post<Encaminhamento>(`/encaminhamentos/${encodeURIComponent(id)}/aprovar`, req);
	}
	...
	arvore(query: ArvoreQuery = {}): Promise<ArvoreNode[]> {
		return this.api.get<ArvoreNode[]>('/encaminhamentos/arvore', query as Record<string, unknown>);
	}
...
class PacientesApi {
	...
	update(id: string, req: AtualizarPacienteRequest): Promise<PacienteCompleto> {
		return this.api.patch<PacienteCompleto>(`/pacientes/${encodeURIComponent(id)}`, req);
	}
	...
	removeCondicaoCronica(pacienteId: string, condicaoId: string): Promise<PacienteCompleto> {
		return this.api.delete<PacienteCompleto>(
			`/pacientes/${encodeURIComponent(pacienteId)}/condicoes-cronicas/${encodeURIComponent(condicaoId)}`
		);
	}
	...
	// ─── Medicamentos ───
	addMedicamento(pacienteId: string, req: CriarMedicamentoRequest): Promise<PacienteCompleto> {
		return this.api.post<PacienteCompleto>(
	...
	removeMedicamento(pacienteId: string, medicamentoId: string): Promise<PacienteCompleto> {
		return this.api.delete<PacienteCompleto>(
			`/pacientes/${encodeURIComponent(pacienteId)}/medicamentos/${encodeURIComponent(medicamentoId)}`
		);
	}
	...
	// ─── Atendimentos ───
	addAtendimento(pacienteId: string, req: CriarAtendimentoRequest): Promise<PacienteCompleto> {
		return this.api.post<PacienteCompleto>(
			`/pacientes/${encodeURIComponent(pacienteId)}/atendimentos`,
			req
		);
	}
	removeAtendimento(pacienteId: string, atendimentoId: string): Promise<PacienteCompleto> {
		return this.api.delete<PacienteCompleto>(
	...
	// ─── Viagens TFD ───
	addViagemTfd(pacienteId: string, req: CriarViagemTfdRequest): Promise<PacienteCompleto> {
		return this.api.post<PacienteCompleto>(
...
class RelatoriosApi {
	...
	async createAndWait(
		...
		throw new ApiError(409, {
			error: { code: 'RELATORIO_NAO_DISPONIVEL', message: 'Timeout aguardando relatório' }
		});
	}
}
...
class AdminApi {
	...
	updateUsuario(id: string, req: AtualizarUsuarioRequest): Promise<CriarUsuarioResponse> {
		return this.api.patch<CriarUsuarioResponse>(`/admin/usuarios/${encodeURIComponent(id)}`, req);
	}
	...
	setAtivoUsuario(id: string, ativo: boolean): Promise<AlterarAtivoResponse> {
		return this.api.post<AlterarAtivoResponse>(`/admin/usuarios/${encodeURIComponent(id)}/ativo`, {
			ativo
		} satisfies AlterarAtivoRequest);
	}
	...
	resetarSenhaUsuario(id: string, novaSenha: string): Promise<void> {
		return this.api.post<void>(`/admin/usuarios/${encodeURIComponent(id)}/reset-senha`, {
			novaSenha
	...
	getBanner(id: string): Promise<AdminBanner> {
		return this.api.get<AdminBanner>(`/admin/sms-banners/${encodeURIComponent(id)}`);
	}
	...
	updateBanner(id: string, req: AtualizarBannerRequest): Promise<AdminBanner> {
		return this.api.patch<AdminBanner>(`/admin/sms-banners/${encodeURIComponent(id)}`, req);
	}

	deleteBanner(id: string): Promise<void> {
		return this.api.delete<void>(`/admin/sms-banners/${encodeURIComponent(id)}`);
	}
	...
	createRecomendacao(req: CriarRecomendacaoRequest): Promise<Recomendacao> {
		return this.api.post<Recomendacao>('/admin/recomendacoes-especialidade', req);
	}

	updateRecomendacao(id: string, req: AtualizarRecomendacaoRequest): Promise<Recomendacao> {
	...
	deleteRecomendacao(id: string): Promise<void> {
		return this.api.delete<void>(`/admin/recomendacoes-especialidade/${encodeURIComponent(id)}`);
	}

	// Configurações
	getConfiguracoes(): Promise<AdminConfiguracoes> {
		return this.api.get<AdminConfiguracoes>('/admin/configuracoes');
	}

	updateConfiguracoes(req: AdminConfiguracoes): Promise<AdminConfiguracoes> {
		return this.api.patch<AdminConfiguracoes>('/admin/configuracoes', req);
	}
	getIntegracoes(): Promise<IntegracoesAdminResponse> {
		return this.api.get<IntegracoesAdminResponse>('/admin/integracoes');
	}

	createIntegracao(req: IntegracaoAdminRequest): Promise<IntegracaoAdminStatus> {
		return this.api.post<IntegracaoAdminStatus>('/admin/integracoes', req);
	}

	salvarIntegracao(req: IntegracaoAdminRequest): Promise<IntegracaoAdminStatus> {
		return this.api.post<IntegracaoAdminStatus>('/admin/integracoes', req);
	}
}
...
class TfdApi {
	...
	veiculos = {
		...
		byId: (id: string): Promise<Veiculo> => this.api.get(`/tfd/veiculos/${encodeURIComponent(id)}`),
	...
	viagens = {
		...
		create: (req: CriarViagemRequest): Promise<ViagemFrota> => this.api.post('/tfd/viagens', req),
	...
	saldo = {
		...
		aportar: (
			...
			opts?: { idempotencyKey?: string }
		): Promise<AporteSaldoFrota[]> => this.api.post('/tfd/saldo/aportar', req, opts),
	...
	saldoAjudaCusto = {
		...
		aportar: (
			...
			opts?: { idempotencyKey?: string }
		): Promise<AporteSaldoAjudaCusto> => this.api.post('/tfd/saldo-ajuda-custo/aportar', req, opts),
	...
	ajudasCusto = {
		...
		pagar: (
			...
			return this.api.postMultipart(`/tfd/ajudas-custo/${encodeURIComponent(id)}/pagar`, fd, opts);
		},
	...
	relatorios = {
		 ...
		 */
		porEspecialidade: (q?: RelatorioEspecialidadeQuery): Promise<RelatorioEspecialidadeResposta> =>
			this.api.get('/tfd/relatorios/especialidades', q as Record<string, unknown> | undefined)
	};
	...
	auditoria = {
		 ...
		 */
		verificar: (prefeituraId?: string): Promise<{ total: number; corrompidos: string[] }> =>
			this.api.get('/tfd/auditoria/verificar', prefeituraId ? { prefeituraId } : undefined),
		/**
	...
	solicitacoesPaciente = {
		...
		aprovar: (
			...
			this.api.post(`/tfd/solicitacoes-paciente/${encodeURIComponent(id)}/aprovar`, req),
		recusar: (id: string, motivo: string): Promise<TfdPacienteSolicAdmin> =>
			this.api.post(`/tfd/solicitacoes-paciente/${encodeURIComponent(id)}/recusar`, {
				motivo
			} satisfies RecusarTfdPacienteSolicRequest),
...
export class CentroRecepcaoApi {
	...
	agendar(id: string, req: AgendarConsultaCentroRequest): Promise<AgendarConsultaCentroResponse> {
	...
	agendarBalcaoRetroativo(
		...
		return this.api.post<AgendamentoBalcaoRetroativoResponse>('/centro/balcao/agendar', req);
	}
	...
	listEscalas(query?: { centro?: string }): Promise<EscalaMedicoCentro[]> {
		return this.api.get<EscalaMedicoCentro[]>(
			'/centro/escalas',
			query as Record<string, unknown> | undefined
		);
	}
	...
	listFilaTriagem(query?: {
		centro?: string;
		data?: string;
		busca?: string;
	}): Promise<ListFilaTriagemResponse> {
		return this.api.get<ListFilaTriagemResponse>(
			'/centro/enfermagem/fila',
			query as Record<string, unknown> | undefined
		);
	}
...
export class CentroGestaoApi {
	...
	obterDashboard(query?: { centro?: string }): Promise<DashboardGestaoCentroResponse> {
		return this.api.get<DashboardGestaoCentroResponse>(
			'/centro/gestao/dashboard',
			query as Record<string, unknown> | undefined
		);
	}
	...
	async listProfissionais(query?: { centro?: string }): Promise<
		Array<{
			id: string;
			nome: string;
	...
	> {
		try {
			...
			>('/centro/gestao/profissionais', query as Record<string, unknown> | undefined);
		...
		try {
			...
			if (Array.isArray(users)) {
				const ehCeo =
					query?.centro?.toUpperCase() === 'CEO' ||
					query?.centro?.toUpperCase() === 'CENTRO_ODONTOLOGICO';
				return users
					.filter((u) => {
					...
					.map((u) => ({
						...
						especialidade:
							u.especialidade || (ehCeo ? 'Odontologia Especializada' : 'Clínica Especializada')
					}));
			}
		} catch (err) {
	...
	atualizarEscala(id: string, req: Partial<EscalaMedicoCentro>): Promise<EscalaMedicoCentro> {
		return this.api.put<EscalaMedicoCentro>(
			`/centro/gestao/escalas/${encodeURIComponent(id)}`,
			req
		);
	}
	...
	excluirEscala(id: string): Promise<{ sucesso: boolean }> {
		return this.api.delete<{ sucesso: boolean }>(
			`/centro/gestao/escalas/${encodeURIComponent(id)}`
		);
	}

	/** Disparar notificações de ausência / mudança de agenda médica aos pacientes (POST /v1/centro/gestao/notificacoes-ausencia). */
	dispararNotificacoesAusencia(req: {
		medicoNome: string;
		dataAfetada: string;
		tipoMotivo: string;
		novaData?: string;
		mensagem: string;
		canais: { app: boolean; sms: boolean; whatsapp: boolean };
	}): Promise<{ sucesso: boolean; totalNotificados: number }> {
		return this.api.post<{ sucesso: boolean; totalNotificados: number }>(
			'/centro/gestao/notificacoes-ausencia',
			req
		);
	}
	...
	obterRelatorioBpa(periodo: string): Promise<RelatorioBpaCentroResponse> {
		return this.api.get<RelatorioBpaCentroResponse>(
			`/centro/gestao/relatorios/bpa?periodo=${encodeURIComponent(periodo)}`
		);
	}
	...
	listAuditoria(
		params?:
			| { limit?: number; offset?: number; acao?: string; centro?: string; busca?: string }
			| number,
		offsetFallback = 0
	): Promise<ListAuditoriaCentroResponse> {
		...
		const qs = q.toString();
		return this.api.get<ListAuditoriaCentroResponse>(
			`/centro/gestao/auditoria${qs ? `?${qs}` : ''}`
		);
	}
	...
	listSalas(query?: { centro?: string }): Promise<SalaConsultorioCentro[]> {
		return this.api.get<SalaConsultorioCentro[]>(
			'/centro/gestao/salas',
			query as Record<string, unknown> | undefined
		);
	}
	...
	atualizarSala(id: string, req: Partial<SalaConsultorioCentro>): Promise<SalaConsultorioCentro> {
		return this.api.put<SalaConsultorioCentro>(
			`/centro/gestao/salas/${encodeURIComponent(id)}`,
			req
		);
	}
	...
	listEspecialidades(query?: { centro?: string }): Promise<EspecialidadeSigtapCentro[]> {
		return this.api.get<EspecialidadeSigtapCentro[]>(
			'/centro/gestao/especialidades',
			query as Record<string, unknown> | undefined
		);
	}
	...
	atualizarEspecialidade(
		id: string,
		req: Partial<EspecialidadeSigtapCentro>
	): Promise<EspecialidadeSigtapCentro> {
		return this.api.put<EspecialidadeSigtapCentro>(
			`/centro/gestao/especialidades/${encodeURIComponent(id)}`,
			req
		);
	}
...
export class CentroWhatsAppApi {
	...
	listarConversas(query?: {
		...
		return this.api.get<ListarConversasResponse>(
			'/centro/whatsapp/conversas',
			query as Record<string, unknown> | undefined
		);
	}
	...
	obterConversa(id: string): Promise<WhatsAppConversaDTO> {
		return this.api.get<WhatsAppConversaDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}`
		);
	}
	...
	assumirConversa(id: string): Promise<WhatsAppConversaDTO> {
		return this.api.post<WhatsAppConversaDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}/assumir`
		);
	}
	...
	transferirConversa(
		id: string,
		req: { novoAtendenteId: string; novoAtendenteNome: string }
	): Promise<WhatsAppConversaDTO> {
		return this.api.post<WhatsAppConversaDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}/transferir`,
			req
		);
	}
	...
	enviarMensagem(id: string, req: EnviarMensagemWhatsAppRequest): Promise<WhatsAppMensagemDTO> {
		return this.api.post<WhatsAppMensagemDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}/mensagens`,
			req
		);
	}
	...
	enviarTemplate(id: string, req: EnviarTemplateWhatsAppRequest): Promise<WhatsAppMensagemDTO> {
		return this.api.post<WhatsAppMensagemDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}/template`,
			req
		);
	}
	...
	finalizarConversa(id: string, req?: { motivo?: string }): Promise<WhatsAppConversaDTO> {
		return this.api.post<WhatsAppConversaDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}/finalizar`,
			req || {}
		);
	}
	...
	atualizarTags(id: string, req: { tags: string[] }): Promise<WhatsAppConversaDTO> {
		return this.api.post<WhatsAppConversaDTO>(
			`/centro/whatsapp/conversas/${encodeURIComponent(id)}/tags`,
			req
		);
	}
}
...
export class UbsApi {
	...
	adicionarFila(
		req: import('./types').AdicionarFilaUbsRequest
	...
	listarFila(query?: {
		...
		return this.api.get<import('./types').ListarFilaUbsResponse>(
			'/ubs/fila-dia',
			query as Record<string, unknown>
		);
	}
	...
	chamarPaciente(
		id: string,
		req?: { consultorio?: string; crm?: string }
	): Promise<{
		...
		return this.api.post<{
			atendimento: import('./types').AtendimentoUbsItem;
			chamada: import('./types').ChamadaPainelUbs;
		}>(`/ubs/fila-dia/${encodeURIComponent(id)}/chamar`, req || {});
	}
	...
	getPainelChamadas(query?: {
		ubsId?: string;
		limite?: number;
	}): Promise<import('./types').PainelChamadasUbsResponse> {
		return this.api.get<import('./types').PainelChamadasUbsResponse>(
			'/ubs/painel/chamadas',
			query as Record<string, unknown>
		);
	}
