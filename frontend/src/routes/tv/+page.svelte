<script lang="ts">
	...
	function dispararChamadaTeste() {
		...
		const teste: ChamadaPainel = {
			id: 'teste-' + Date.now(),
			pacienteNome: ehCeo
				? 'MARIA SILVA SANTOS (TESTE DE ÁUDIO)'
				: 'JOÃO CARLOS PEREIRA (TESTE DE ÁUDIO)',
	...
	onMount(() => {
		...
		const pinUrl =
			page.url.searchParams.get('centro') ||
			page.url.searchParams.get('pin') ||
			page.url.searchParams.get('senha');
		if (pinUrl) {
...
<svelte:head>
	<title
		>{centroPareado
			? `${centroPareado.sigla} · Painel TV de Chamadas`
			: 'UNISISM · Conectar Painel TV'}</title
	>
</svelte:head>

<div
	class="flex min-h-screen flex-col justify-between overflow-hidden bg-slate-950 font-sans text-white select-none"
>
	{#if !centroPareado}
		<!-- ═══════════════════════════════════════════════════════════════
		...
		<div class="flex flex-1 flex-col items-center justify-center p-6 md:p-12">
			<div
				class="w-full max-w-xl border-2 border-slate-800 bg-slate-900 p-8 shadow-[8px_8px_0_rgba(15,23,42,0.6)] md:p-10"
			>
				<!-- Header institucional -->
				<div class="flex items-center gap-4 border-b border-slate-800 pb-6">
					<div
						class="flex h-12 w-12 items-center justify-center border-2 border-white bg-blue-900 font-mono text-xl font-bold text-white"
					>
						U
					</div>
					<div>
						...
						<h1 class="mt-0.5 font-mono text-xl font-bold tracking-tight text-white uppercase">
				...
				<div class="mt-8">
					<label
						for="senha-tv"
						class="mb-2 block font-mono text-[10px] font-semibold tracking-widest text-slate-300 uppercase"
					>
					...
					{#if erroAutenticacao}
						<div
							class="mt-3 border border-red-700 bg-red-950/80 px-3 py-2 font-mono text-[11px] font-bold tracking-wider text-red-300 uppercase"
						>
							⚠ {erroAutenticacao}
						</div>
					{/if}
					...
					<button
						...
						class="mt-6 flex w-full items-center justify-center gap-2 border border-blue-900 bg-blue-900 py-3 font-mono text-xs font-bold tracking-widest text-white uppercase transition-colors hover:bg-blue-950 active:bg-blue-950"
					>
					...
					<div class="mt-8 border-t border-slate-800 pt-6">
						<div
							class="mb-4 text-center font-mono text-[10px] font-bold tracking-widest text-slate-400 uppercase"
						>
						...
						<div class="grid grid-cols-2 gap-3">
							<button
								...
								class="group border border-slate-800 bg-slate-950 p-4 text-left transition-colors hover:border-blue-500 hover:bg-blue-950/40"
							>
								<div class="flex items-center justify-between">
									<span
										class="font-mono text-xs font-bold text-blue-300 uppercase group-hover:text-white"
										>CEM</span
									>
									<span class="inline-block h-1.5 w-1.5 bg-blue-400"></span>
								</div>
								<div class="mt-1 font-sans text-xs text-slate-300">Especialidades Médicas</div>
								<div class="mt-2 font-mono text-[10px] font-bold text-blue-400 uppercase">
									SENHA: CEM-2026
								</div>
							</button>
							...
							<button
								...
								class="group border border-slate-800 bg-slate-950 p-4 text-left transition-colors hover:border-emerald-500 hover:bg-emerald-950/40"
							>
								<div class="flex items-center justify-between">
									<span
										class="font-mono text-xs font-bold text-emerald-300 uppercase group-hover:text-white"
										>CEO</span
									>
									<span class="inline-block h-1.5 w-1.5 bg-emerald-400"></span>
								</div>
								<div class="mt-1 font-sans text-xs text-slate-300">Especialidades Odonto</div>
								<div class="mt-2 font-mono text-[10px] font-bold text-emerald-400 uppercase">
									SENHA: CEO-2026
								</div>
							</button>
						</div>
	...
	{:else}
		...
		>
			<div class="flex items-center gap-4">
				<div
					class="flex h-12 w-12 items-center justify-center border-2 border-white {centroPareado.corTema ===
					'blue'
						? 'bg-blue-900 text-white'
						: 'bg-emerald-900 text-white'} font-mono text-lg font-bold"
				>
					{centroPareado.sigla}
				</div>
				<div>
			...
			<div class="flex items-center gap-6">
				<div class="text-right font-mono">
					<div class="text-2xl font-bold tracking-tight text-white md:text-3xl">
						{horarioAtual}
					</div>
				...
				<div
					class="flex items-center gap-2 border-l border-slate-800 pl-6 font-mono text-[10px] uppercase"
				>
					<button
						onclick={dispararChamadaTeste}
						class="border border-slate-700 bg-slate-800 px-3 py-1.5 font-bold tracking-wider text-slate-200 transition-colors hover:bg-slate-700"
					>
						🔊 Testar Som
					</button>
					...
					<button
						onclick={desparear}
						class="border border-red-800 bg-red-950 px-3 py-1.5 font-bold tracking-wider text-red-300 transition-colors hover:bg-red-900"
					>
						Desconectar
					</button>
				</div>
		...
		<main class="grid flex-1 grid-cols-12 items-stretch gap-6 overflow-hidden p-6">
			<!-- COLUNA PRINCIPAL: PACIENTE CHAMADO EM DESTAQUE (COL-8) -->
			<div class="col-span-12 flex flex-col justify-between lg:col-span-8">
				<div
					class="relative flex-1 border-2 {piscarDestaque
						? centroPareado.corTema === 'blue'
							? 'border-blue-400 bg-blue-950/80'
							: 'border-emerald-400 bg-emerald-950/80'
						: 'border-slate-800 bg-slate-900'} flex flex-col justify-between p-8 transition-colors duration-300 md:p-12"
				>
					...
					<div class="flex items-center justify-between border-b border-slate-800 pb-4">
						<div class="flex items-center gap-3">
							{#if chamadaAtual?.tipo === 'TRIAGEM'}
								<span
									class="inline-flex animate-pulse items-center gap-2 border border-purple-500 bg-purple-950 px-3 py-1 font-mono text-xs font-bold tracking-widest text-purple-200 uppercase"
								>
							...
							{:else}
								<span
									class="inline-flex items-center gap-2 border {piscarDestaque
										? 'border-amber-600 bg-amber-50 text-amber-900'
										: 'border-blue-700 bg-blue-950 text-blue-300'} px-3 py-1 font-mono text-xs font-bold tracking-widest uppercase"
								>
									{#if piscarDestaque}
							...
							<span class="font-mono text-xs tracking-wider text-slate-400 uppercase">
								Horário: <strong class="font-mono text-white"
									>{chamadaAtual?.horario || horarioAtual}</strong
								>
							</span>
						</div>
					...
					<div class="my-auto py-6">
						<div
							class="mb-2 font-mono text-[11px] font-bold tracking-widest text-blue-400 uppercase"
						>
							PACIENTE CONVOCADO:
						</div>
						<div
							class="font-sans text-4xl leading-tight font-bold tracking-tight text-white uppercase md:text-6xl xl:text-7xl"
						>
					...
					<div class="grid grid-cols-1 gap-4 border-t border-slate-800 pt-6 md:grid-cols-2">
						<div
							class="border-2 {centroPareado.corTema === 'blue'
								? 'border-blue-800 bg-blue-950'
								: 'border-emerald-800 bg-emerald-950'} p-6"
						>
							<div
								class="font-mono text-[10px] font-bold tracking-widest {centroPareado.corTema ===
								'blue'
									? 'text-blue-300'
							...
							<div
								class="mt-1 font-mono text-2xl font-bold tracking-wide text-white uppercase md:text-3xl"
							>
								{chamadaAtual?.consultorio || 'SALA DE ATENDIMENTO'}
							</div>
						</div>

						<div class="flex flex-col justify-center border border-slate-800 bg-slate-950 p-6">
							<div class="font-mono text-[10px] font-bold tracking-widest text-slate-400 uppercase">
								PROFISSIONAL / ESPECIALIDADE:
							</div>
							<div class="mt-1 font-sans text-lg font-bold text-white">
								{chamadaAtual?.medicoNome || 'Corpo Clínico'}
							</div>
							<div class="mt-0.5 font-mono text-xs font-semibold text-blue-400 uppercase">
			...
			<div class="col-span-12 flex flex-col border border-slate-800 bg-slate-900 p-6 lg:col-span-4">
				...
				<div class="flex flex-1 flex-col gap-3 overflow-hidden">
					{#if ultimasChamadas.length === 0}
						<div
							class="flex flex-1 items-center justify-center border border-slate-800 p-6 text-center font-sans text-xs text-slate-500 uppercase"
						>
					...
					{:else}
						{#each ultimasChamadas as uc}
							<div
								class="border border-slate-800 bg-slate-950 p-3.5 transition-colors hover:border-slate-700"
							>
								<div class="flex items-center justify-between">
									<span
										class="max-w-[200px] truncate font-sans text-xs font-bold text-white uppercase"
									>
										{uc.pacienteNome}
									</span>
								...
								<div
									class="mt-2 flex items-center justify-between border-t border-slate-800 pt-2 font-mono text-[10px]"
								>
									<span
										class="font-bold {centroPareado.corTema === 'blue'
											? 'text-blue-400'
											: 'text-emerald-400'} uppercase"
									>
										{uc.consultorio}
									</span>
									<span class="max-w-[120px] truncate text-slate-400 uppercase">
										{uc.especialidade}
									</span>
								</div>
							</div>
				...
				<!-- AVISO DE ORIENTAÇÃO -->
				<div
					class="mt-4 border-l-4 border-blue-900 bg-slate-950 p-3 font-mono text-[10px] text-slate-300"
				>
					<div class="mb-1 flex items-center gap-1.5 font-bold text-blue-300 uppercase">
						<span class="inline-block h-1.5 w-1.5 bg-blue-400"></span>
						ORIENTAÇÃO AO USUÁRIO
					</div>
					<p class="font-sans text-[11px] leading-relaxed text-slate-300">
						Ao ser anunciado no painel, dirija-se à porta do consultório indicado portando documento
		...
		<footer
			class="flex items-center justify-between border-t border-slate-800 bg-slate-900 px-6 py-2.5 font-mono text-xs"
		>
			<div class="flex items-center gap-4 overflow-hidden">
				<span
					class="border border-slate-700 bg-slate-800 px-2 py-0.5 text-[9px] font-bold tracking-widest whitespace-nowrap text-blue-300 uppercase"
				>
					INFORME SUS
				</span>
				<div class="truncate text-[11px] tracking-wider text-slate-300 uppercase">
					Secretaria Municipal de Saúde · Mantenha seus documentos em mãos · Vacinação disponível na
					UBS · Dúvidas, procure a recepção.
				</div>
			</div>

			<div class="pl-4 font-mono text-[10px] whitespace-nowrap text-slate-500 uppercase">
				UNISISM TV v1.0
			</div>
		</footer>
	{/if}
