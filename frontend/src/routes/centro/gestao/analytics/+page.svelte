<script lang="ts">
	...
	let concluidos = $derived(
		encaminhamentos.filter((e) => (e as any).statusAtendimentoCentro === 'CONCLUIDO').length
	);
	...
	let porEspecialidade = $derived.by(() => {
		...
		for (const e of encaminhamentos) {
			const esp =
				e.solicitacao?.especialidadeSolicitada || (ehCeo ? 'Endodontia' : 'Clínica Geral');
...
<div class="flex flex-col gap-5 font-mono text-xs">
	...
	{:else}
		...
		<section class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
			<div class="border border-slate-200 bg-white p-4">
				<div class="text-[10px] font-bold tracking-widest text-slate-500 uppercase">
					VOLUME TOTAL REGULADO
				</div>
				<div class="mt-2 font-sans text-3xl font-extrabold text-slate-900">{totalAtendimentos}</div>
				<div class="mt-1 font-mono text-[10px] text-slate-500">Encaminhamentos processados</div>
			</div>
			...
			<div class="border border-slate-200 bg-white p-4">
				...
				<div class="mt-2 font-sans text-3xl font-extrabold text-emerald-700">
					{taxaComparecimento}%
				</div>
				<div class="mt-1 font-mono text-[10px] font-bold text-emerald-800">
					Aderência aos agendamentos
				</div>
			</div>
			...
			<div class="border border-slate-200 bg-white p-4">
				...
				<div
					class="mt-2 text-3xl font-extrabold {ehCeo
						? 'text-emerald-800'
			...
			<div class="border border-slate-200 bg-white p-4">
				...
				<div class="mt-2 font-sans text-3xl font-extrabold text-rose-700">
					{totalAtendimentos > 0 ? Math.round((faltas / totalAtendimentos) * 100) : 0}%
				</div>
				<div class="mt-1 font-mono text-[10px] font-bold text-rose-800">
		...
		<section class="grid grid-cols-1 gap-5 lg:grid-cols-12">
			...
			<div class="flex flex-col gap-4 border border-slate-200 bg-white p-5 lg:col-span-7">
				<div class="flex items-center justify-between border-b border-slate-200 pb-3">
					<span class="font-bold text-slate-900 uppercase"
						>DESEMPENHO POR ESPECIALIDADE ({siglaOrgao})</span
					>
				...
				<div class="flex flex-col gap-3">
					{#each porEspecialidade as esp}
						<div class="flex flex-col gap-1.5 border border-slate-100 bg-slate-50 p-3">
							<div class="flex items-center justify-between font-sans">
								<span class="text-xs font-bold text-slate-900">{esp.nome}</span>
								<span class="font-mono text-xs font-bold text-slate-700"
									>{esp.total} atendimentos</span
								>
							</div>

							<!-- Barra de progresso -->
							<div class="h-2 w-full overflow-hidden bg-slate-200">
								<div
									class="h-full {ehCeo ? 'bg-emerald-700' : 'bg-blue-900'} transition-all"
									style="width: {totalAtendimentos > 0
										? (esp.total / totalAtendimentos) * 100
										: 0}%"
								></div>
							</div>

							<div
								class="mt-0.5 flex items-center justify-between font-mono text-[10px] text-slate-500"
							>
					...
					{:else}
						<div class="p-6 text-center text-slate-500">
							Nenhum atendimento especializado registrado no período.
						</div>
					{/each}
				</div>
			</div>
			...
			<div class="flex flex-col gap-4 border border-slate-200 bg-white p-5 lg:col-span-5">
				<div class="flex items-center justify-between border-b border-slate-200 pb-3">
				...
				<div class="flex flex-col gap-3">
					{#each porUbsOrigem as ubs}
						<div class="flex flex-col gap-1">
							<div class="flex items-center justify-between font-sans text-xs">
								<span class="truncate font-semibold text-slate-800">{ubs.nome}</span>
					...
					{:else}
						<div class="p-6 text-center text-slate-500">
							Nenhuma UBS vinculada com demandas ativas.
						</div>
					{/each}
				</div>
			</div>
