<script lang="ts">
	...
	import type { IntegracoesAdminResponse } from '$lib/api';
	...
	type IntegracaoStatus = NonNullable<IntegracoesAdminResponse['integracoes']>[number];

	let loading = $state(true);
	let errorMsg = $state<string | null>(null);
	let response = $state<IntegracoesAdminResponse | null>(null);
	...
	async function carregarStatus() {
		...
		try {
			...
			if (e instanceof ApiError) {
				...
				errorMsg =
					e instanceof Error ? e.message : 'Erro desconhecido ao carregar status das integrações.';
			}
		} finally {
			loading = false;
		}
	...
	async function salvarConector() {
		...
		try {
			...
			formErro = e instanceof Error ? e.message : 'Falha ao salvar conector.';
		} finally {
			formEnviando = false;
		}
	}
	...
	let federais = $derived(
		(response?.integracoes ?? [])
			.filter((i) => i.tipo && i.tipo.toUpperCase() === 'FEDERAL')
			.filter(isReal) || []
	);
...
{#if auth.me?.role === 'DESENVOLVEDOR' || auth.me?.role === 'ADMIN'}
	<section class="flex flex-col gap-6">
		<!-- Top Bar / Header -->
		<div
			class="flex flex-wrap items-center justify-between gap-4 border-t-2 border-b border-slate-200 border-t-slate-800 bg-white p-4 shadow-sm"
		>
			<div>
				...
				<p class="text-xs text-slate-500">
					Status em tempo real das conexões externas e internas do ecossistema UniSISM
				</p>
			</div>
			<div class="flex gap-2">
				...
				/>
				<PrimaryButton label="+ Configurar Conector Real" variant="primary" onclick={abrirModal} />
				<button
		...
		{#if loading && !response}
			<div
				class="flex flex-col items-center justify-center border border-slate-200 bg-white py-20 text-center shadow-sm"
			>
				<div
					class="h-10 w-10 animate-spin rounded-full border-4 border-slate-200 border-t-blue-600"
				></div>
				<div class="mt-4 font-mono text-xs font-bold tracking-wider text-slate-700">
					TESTANDO CONECTORES
				</div>
				<p class="mt-2 text-xs text-slate-500">
					Acessando endpoints de saúde, verificando latência de rede e integridade de storage...
				</p>
			</div>
		...
		{:else if errorMsg}
			<div class="border border-red-200 bg-red-50 p-6 text-center shadow-sm">
				<div
					class="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full border border-red-300 bg-red-100 text-red-600"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
				...
				<h2 class="font-mono text-sm font-bold text-red-900">
					Falha na Comunicação com o Servidor
				</h2>
				<p class="mt-2 text-xs text-red-700">{errorMsg}</p>
				<div class="mt-4 flex justify-center gap-2">
					<PrimaryButton label="Tentar Novamente" variant="primary" onclick={carregarStatus} />
				</div>
			</div>
		...
		{:else if response}
			...
			<div class="shadow-sm">
				{#if response.status === 'online'}
					<div class="flex items-center gap-4 border border-emerald-200 bg-emerald-50/50 p-4">
						...
						<div class="flex-1">
							<h3 class="font-mono text-xs font-bold tracking-wider text-emerald-950 uppercase">
								Todos os Conectores Saudáveis
							</h3>
							<p class="text-xs text-emerald-800">
								Todas as integrações locais e federais respondendo normalmente.
							</p>
						</div>
						<div class="font-mono text-[10px] text-emerald-700">
				...
				{:else if response.status === 'degraded'}
					<div class="flex items-center gap-4 border border-amber-200 bg-amber-50/50 p-4">
						...
						<div class="flex-1">
							<h3 class="font-mono text-xs font-bold tracking-wider text-amber-950 uppercase">
								Instabilidade Detectada
							</h3>
							<p class="text-xs text-amber-800">
								Um ou mais conectores estão offline ou demorando para responder.
							</p>
						</div>
				...
				{:else}
					<div class="flex items-center gap-4 border border-red-200 bg-red-50/50 p-4">
						...
						<div class="flex-1">
							<h3 class="font-mono text-xs font-bold tracking-wider text-red-950 uppercase">
								Falha Crítica
							</h3>
							<p class="text-xs text-red-800">
								Nenhum conector está respondendo. Verifique os logs do backend.
							</p>
						</div>
						<div class="font-mono text-[10px] text-red-700">
							Último teste: {formatTime(response.checkedAt)}
						</div>
					</div>
				{/if}
			...
			<div class="border border-slate-200 bg-white shadow-sm">
				...
				<div class="grid grid-cols-1 gap-px bg-slate-200 md:grid-cols-3">
					{#each federais as item (item.nome)}
						<div
							class="flex min-h-[140px] flex-col justify-between bg-white p-5 transition-colors hover:bg-slate-50/50"
						>
							<div>
								<div class="flex items-start justify-between gap-4">
									...
									<div class="flex items-center gap-1.5">
										{#if item.status === 'online'}
											...
											<span
												class="font-mono text-[9px] font-bold tracking-wider text-emerald-700 uppercase"
												>ONLINE</span
											>
										{:else}
											<span class="inline-block h-2 w-2 rounded-full bg-rose-500"></span>
											<span
												class="font-mono text-[9px] font-bold tracking-wider text-rose-700 uppercase"
												>OFFLINE</span
											>
										{/if}
									</div>
							...
							<div
								class="mt-4 flex items-center justify-between gap-2 border-t border-slate-100 pt-3"
							>
								<span
									class="max-w-[70%] truncate font-mono text-[10px] text-slate-500"
									title={item.mensagem}>{item.mensagem}</span
								>
								{#if item.status === 'online'}
									<span
										class="border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-emerald-800"
									>
										{item.latencyMs} ms
									</span>
								{:else}
									<span
										class="border border-red-200 bg-red-50 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-red-800"
									>
										—
									</span>
								{/if}
					...
					{:else}
						<div
							class="bg-white p-8 text-center text-slate-500 text-xs font-mono col-span-3 border border-slate-100"
						>
			...
			<div class="border border-slate-200 bg-white shadow-sm">
				...
				<div class="grid grid-cols-1 gap-px bg-slate-200 md:grid-cols-2 lg:grid-cols-4">
					{#each internos as item (item.nome)}
						<div
							class="flex min-h-[140px] flex-col justify-between bg-white p-5 transition-colors hover:bg-slate-50/50"
						>
							<div>
							...
							>
								<span
									...
									title={item.mensagem}>{item.mensagem}</span
								>
								{#if item.status === 'online' && (item.latencyMs ?? 0) > 0}
									<span
										class="border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-emerald-800"
									>
										{item.latencyMs} ms
								...
								{:else if item.status === 'online'}
									<span
										class="border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-slate-600"
									>
										ativo
									</span>
								{:else}
					...
					{:else}
						<div
							class="bg-white p-8 text-center text-slate-500 text-xs font-mono col-span-4 border border-slate-100"
						>
		...
		{#if modalAberto}
			...
			>
				<div class="flex flex-col gap-4 p-1">
					{#if formErro}
						<div
							class="border border-red-700 bg-red-50 px-3 py-2 font-mono text-[10px] font-bold text-red-900 uppercase"
						>
							⚠ {formErro}
						</div>
					{/if}
